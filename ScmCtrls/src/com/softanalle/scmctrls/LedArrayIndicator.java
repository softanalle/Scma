...
public class LedArrayIndicator extends View {
	private boolean isInitialized = false;
	private static String TAG = "LedArrayIndicator";
	...
	*/
	...
	private static int mLedCount = 0;

	public LedArrayIndicator(Context context) {
		super(context);
		// TODO Auto-generated constructor stub
		if (!isInitialized) initializeFirstTime();
	}
	public LedArrayIndicator(Context context, AttributeSet attrs) {
		super(context, attrs);
		if (!isInitialized) initializeFirstTime();
	}

	public LedArrayIndicator(Context context, AttributeSet attrs, int defStyle) {
	...
	public boolean setConfig(int colors[]) {
		if ( colors.length > 0) {
		mLedCount = colors.length;
		mRectColors = new int[mLedCount];
